...
`2` we're saying "look at `theme.space`, find the key for "2" and return the
value - which equals `20px`. And with TypeScript, we get really nice
intellisense as well as type-checking of the values passed into the prop. At
scale and across a big dev team, this works well after the basics are
understood.
...
`flexbox` and others, each providing their own set of props that we can then tap
